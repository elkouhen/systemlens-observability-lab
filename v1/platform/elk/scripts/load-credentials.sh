...
export KIBANA_URL="${KIBANA_URL:-https://kibana.observability.test}"
_credentials_namespace="${ELASTIC_NAMESPACE:-elastic-stack}"
_credentials_app_namespace="${APP_NAMESPACE:-h0tl-supermarche-app}"
_credentials_resolve="${ELASTICSEARCH_CURL_RESOLVE:-elasticsearch.observability.test:443:127.0.0.1}"
...
APM_SECRET_TOKEN="$(kubectl -n "${_credentials_namespace}" get secret apm-server-apm-token \
  -o jsonpath='{.data.secret-token}' 2>/dev/null | base64 --decode || true)"
