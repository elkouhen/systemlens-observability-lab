...
Le manifest déclare les skills `systemlens`, `documentation-writer` et
`kibana-dashboards`. Pour ajouter une dépendance, utiliser
`apm install <source>`, vérifier le diff produit, puis committer `apm.yml` et
`apm.lock.yaml` si APM génère ce lockfile. Ne jamais ajouter un serveur MCP sans
valider sa source, ses permissions et son besoin pour le projet.
